...
--- @field comboBrewWithJellyfish?          boolean                                                     If true, drink a Saradomin brew in the same tick a jellyfish is eaten (default: false)
--- @field brewSipsPerRestore?             integer                                                     Drink a prayer/restore potion after this many Saradomin brew sips to restore brew-drained stats (0 disables, default: 3)
...
function PlayerManager.new(config)
    ...
    self.config.comboBrewWithJellyfish = (config and config.comboBrewWithJellyfish) or false
    self.config.brewSipsPerRestore = (config and config.brewSipsPerRestore) or 3
    ...
    -- Number of Saradomin brew sips taken since the last stat-restoring sip
    self.brewSips = 0
...
function PlayerManager:oneTickEat(solidFoodThreshold, jellyfishThreshold, healingPotionThreshold)
    ...
    if self.config.preferJellyfishHighAdrenaline then
        ...
        if currentAdrenaline >= self.config.adrenalineThreshold and hasJellyfish and jellyfishThreshold then
            if self:_eat("jellyfish") then
                ...
                -- Combo a Saradomin brew in the same tick to survive longer
                self:_comboBrew(currentTick)
            end
    ...
    if healingPotionThreshold then
        local potions = self:_filterFoodItems("potion")
        if #potions > 0 then
            if self:_eat("potion") then
                self.timestamps.drink = currentTick
                success = true
                -- Count it toward the restore cycle if it was a brew
                if potions[1].name:find("Saradomin brew") then
                    self.brewSips = self.brewSips + 1
                end
            end
        end
    end
...
function PlayerManager:managePrayer()
    ...
    -- combat stats brews lower). Fires independently of the prayer threshold.
    if self.config.brewSipsPerRestore > 0 and
        self.brewSips >= self.config.brewSipsPerRestore and
        #self.inventory.prayer > 0 then
        if self:drink() then
            self.brewSips = 0
        end
        return
...
function PlayerManager:updateConfig(config)
    ...
    if config.comboBrewWithJellyfish ~= nil then
        self.config.comboBrewWithJellyfish = config.comboBrewWithJellyfish
    end

    if config.brewSipsPerRestore then
        self.config.brewSipsPerRestore = config.brewSipsPerRestore
    end
...
--- Drinks a Saradomin brew in the same tick as eating (combo eat) to maximise
--- healing and survivability. Matches both "Saradomin brew" and "Super Saradomin
--- brew". No-op if disabled, on the drink cooldown, or no brew is available.
--- @param currentTick integer: The tick the combo is happening on
--- @return boolean: Whether a brew was drunk
--- @private
function PlayerManager:_comboBrew(currentTick)
    if not self.config.comboBrewWithJellyfish then return false end

    -- Respect the shared drink cooldown so we don't waste/spam brews
    if not self:_checkCooldown(self.timestamps.drink) then return false end

    for _, potion in ipairs(self:_filterFoodItems("potion")) do
        if potion.name:find("Saradomin brew") then
            if Inventory:DoAction(potion.id, 1, API.OFF_ACT_GeneralInterface_route) then
                self.timestamps.drink = currentTick
                self.brewSips = self.brewSips + 1
                Utils:log(("Combo: drank %s with jellyfish (brew sip %d)"):format(potion.name, self.brewSips), "debug")
                return true
            end
        end
    end
