--[[
    Author:      Valtrex
    Version:     2.2 (Refactored MatheusArcanjjo)
...
local version           = "2.26_Final_Refactor"
local Rune_type         = ""
local Rune              = ""
local Familier          = "None"
...
local aioFamiliar = {
    { name = "Abyssal parasite", FAMILIARID = 12035, ABNAME = API.GetABs_name("Abyssal parasite pouch", true) },
    { name = "Abyssal lurker",   FAMILIARID = 12037, ABNAME = API.GetABs_name("Abyssal lurker pouch", true) },
    { name = "Abyssal titan",    FAMILIARID = 12796, ABNAME = API.GetABs_name("Abyssal titan pouch", true) },
}
...
local function teleportToDestination(destination, isLodestone)
    ...
    local teleportAbility = (id ~= nil) and getABS_id(id, destinationStr) or API.GetABs_name(destinationStr, true)
...
local function teleportToEdgeville()
    local ws = API.GetABs_name("Wilderness sword", true)
    if ws and ws.enabled and ws.action == "Edgeville" then
...
local function teleportToHauntHill()
    local hh = API.GetABs_name("Passing bracelet", true)
...
local function TeleportWarRetreat()
    local wrt = API.GetABs_name("War's Retreat Teleport", true)
    if wrt and wrt.enabled then
        API.logDebug("Info: Teleport to War's Retreat")
        API.logInfo("Teleport to War's Retreat.")
        API.DoAction_Ability_Direct(wrt, 1, API.OFF_ACT_GeneralInterface_route)
        API.RandomSleep2(2000,1000,2000)
        API.WaitUntilMovingEnds(15, 10) 
    else
...
local function teleportToUM()
    local um = API.GetABs_name("Underworld Grimoire", false) or API.GetABs_name("Underworld Grimoire 2", false) or API.GetABs_name("Underworld Grimoire 3", false) or API.GetABs_name("Underworld Grimoire 4", false)
    if um and um.enabled then
...
local function findPowerburst()
    local powerbursts = Inventory:GetItems() 
    for _, item in ipairs(powerbursts) do
        for _, powerburstId in ipairs(ID_Items.POWERBURST) do
            if item.id == powerburstId then
                return item.id 
            end
        end
    end
...
local function OpenEquipmentIfNeeded()
    if not Equipment:IsOpen() then
         API.DoAction_Interface(0xc2,0xffffffff,1,1432,4,1,API.OFF_ACT_GeneralInterface_route);
...
local function RenewFamiliar()
    ...
    if isAtLocation(AREA.WARETREAT, 50)then
        if Familiars:GetSummoningPoints() < 40 then 
        ...
        if Inventory:GetItemAmount(selectedFamiliar) < 1 then 
...
local function RuneCounters()
    ...
    -- ESlot Enum: HELM=0, BODY=4, BOTTOM=6, GLOVES=7, BOOTS=8
    if Equipment:GetSlotData(ESlot.HEAD).id == 32357 and Equipment:GetSlotData(ESlot.BODY).id == 32581 and Equipment:GetSlotData(ESlot.BOTTOM).id == 32582 and Equipment:GetSlotData(ESlot.GLOVES).id == 32360 and Equipment:GetSlotData(ESlot.BOOTS).id == 32361 then
        API.logDebug("Found:  Infinity ethereal outfit")
        runecount = runecount + 12
    end
    if Equipment:GetSlotData(ESlot.HEAD).id == 32347 and Equipment:GetSlotData(ESlot.BODY).id == 32348 and Equipment:GetSlotData(ESlot.BOTTOM).id == 32349 and Equipment:GetSlotData(ESlot.GLOVES).id == 32350 and Equipment:GetSlotData(ESlot.BOOTS).id == 32351 then
        API.logDebug("Found:  Blood ethereal outfit")
        runecount = runecount + 6
    end
    if Equipment:GetSlotData(ESlot.HEAD).id == 32352 and Equipment:GetSlotData(ESlot.BODY).id == 32353 and Equipment:GetSlotData(ESlot.BOTTOM).id == 32354 and Equipment:GetSlotData(ESlot.GLOVES).id == 32355 and Equipment:GetSlotData(ESlot.BOOTS).id == 32356 then
        API.logDebug("Found:  Death ethereal outfit")
        runecount = runecount + 6
    end
    if Equipment:GetSlotData(ESlot.HEAD).id == 32342 and Equipment:GetSlotData(ESlot.BODY).id == 32343 and Equipment:GetSlotData(ESlot.BOTTOM).id == 32344 and Equipment:GetSlotData(ESlot.GLOVES).id == 32345 and Equipment:GetSlotData(ESlot.BOOTS).id == 32346 then
        API.logDebug("Found:  Law ethereal outfit")
        runecount = runecount + 6
    end

    if Inventory:Contains(24205) then
        API.logDebug("Found: Massive pouch")
        runecount = runecount + 18
    end
    if Inventory:Contains(5514) then
        API.logDebug("Found: Giant pouch")
        runecount = runecount + 12
    end
    if Inventory:Contains(5512) then
        API.logDebug("Found: Large pouch")
        runecount = runecount + 9
    end
    if Inventory:Contains(5510) then
        API.logDebug("Found: Medium pouch")
        runecount = runecount + 6
    end
    if Inventory:Contains(5509) then
        API.logDebug("Found: Small pouch")
        runecount = runecount + 3
    end
    API.logDebug("Total essence capacity from gear/pouches: " .. runecount)
    ...
    API.logDebug("Total essence capacity with familiar: " .. runecount)
end
--------------------SOUL ALTAR------------------
-----------------------PVP----------------------
...
local function invCheck()
    ...
    if selectedFamiliar then
        local wrt = API.GetABs_name("War's Retreat Teleport", true)
        local warCheck = wrt and wrt.enabled
...
local function RuneToCraft()
    ...
    if (aioSelectF.string_value == "Abyssal parasite") then      Familier = "Abyssal parasite"
    elseif(aioSelectF.string_value == "Abyssal lurker") then        Familier = "Abyssal lurker"
    elseif(aioSelectF.string_value == "Abyssal titan") then         Familier = "Abyssal titan"
    end
end
---------------------CHECKS---------------------
-----------------NORMAL FUNCTIONS---------------
...
local function DoMage()
    if API.DoAction_NPC(0x29, API.OFF_ACT_InteractNPC_route, { ID_NPC.MAGE }, 50) then
        API.logDebug("Doaction: Mage of Zamorak. Aguardando a teleportação...")
        
        local timeout = 0
        repeat
            API.Sleep_tick(1) 
            timeout = timeout + 1
        until not isAtLocation(AREA.WILDY, 50) or timeout > 15 
        
        if timeout > 15 then
            API.logError("Falha ao teleportar para o Abismo. Tempo de espera excedido.")
        else
            API.logInfo("Teleportado para o Abismo com sucesso.")
        end
    end
end
...
local function CraftRune()
    if not API.ReadPlayerMovin2() then
        if canUsePowerburst() and findPowerburst() then
            ...
            return Inventory:Use(findPowerburst())
        end
        ...
        if Necro == false then
            if invContains(ID_Items.ESSENCE) then
                ...
                if Counter == true then
                    Trips = Trips + 1 
                    Runes = Runes + Inventory:GetItemAmount(selectedRune)
                    Counter = false
                end
    ...
    if not (API.ReadPlayerAnim() == ID_Anim.Teleport_DOWN) and not API.ReadPlayerMovin2() and (not API.CheckAnim(50)) then
        ...
        if Necro == false then
            ...
            if Inventory:GetItemAmount(55667) >= 16  or Inventory:GetItemAmount(7936) >= 16 or Inventory:GetItemAmount(18178) >= 16 then
                if not API.ReadPlayerMovin2() then
                    ...
                    API.DoAction_Object1(0xb5, API.OFF_ACT_GeneralObject_route0, ID_Object.WILDY_WALL, 65)
        ...
        elseif Necro == true then
            if CheckforImpureEssence() then
                ...
                elseif Equipment:GetSlotData(ESlot.GLOVES).id == 56416 then
                    API.logDebug("Item found: Passing Braclet in glove slot.")
                    API.DoAction_Ability_Direct(API.GetABs_name("Passing bracelet", true), 3, API.OFF_ACT_GeneralInterface_route)
                else
                    API.logDebug("No Passing Braclet found, walking.")
                    API.DoAction_Object1(0x39, API.OFF_ACT_GeneralObject_route0, {ID_Object.DARK_PORTAL}, 50)
                end
                banking = 0
                fail = 0
...
while API.Read_LoopyLoop() do
    ...
    local TripsPH = (elapsedMinutes > 0) and round((Trips * 60) / elapsedMinutes) or 0
    local RunesPH = (elapsedMinutes > 0) and round((Runes * 60) / elapsedMinutes) or 0

    local metrics = {
    {"Script","AIO Runecrafter - (v" .. version .. ")"},
    {"Rune type:", Rune_type},
    {"Rune:", Rune},
    {"Familier:", Familier},
    {"Trips:", formatNumber(Trips).." ("..formatNumber(TripsPH).."/H)"},
    {"Runes:",formatNumber(Runes).." ("..formatNumber(RunesPH).."/H)"},
    }
...
---------------- UI
    ...
    if scriptPaused == true then
        if btnStart.return_click then
            ...
            MAX_IDLE_TIME_MINUTES = (tickJagexAcc.box_ticked and 5 or 15)
            scriptPaused = false
    ...
    if isBankpinInterfacePresent() then
        ...
        if selectedFamiliar then
            ...
            if familiarrenew == 1 and not API.isProcessing() then
                RenewFamiliar()
            elseif not Familiars:HasFamiliar() and not API.isProcessing() then
