--[[
    ...
    Updated:     03-06-2025 (some stuff still broken due to API changes, fix shit you need. Added astrals)
    Script:      Runecrafter
...
local Auras = require("libraries.Auras")
local Bank = require("libraries.bank")
local CreationInterface = require("libraries.CreationInterface")
local Equipment = require("libraries.Equipment")
local InputText = require("libraries.InputText")
local PrayerFlicker = require("libraries.prayer_flicker")
local Shop = require("libraries.shop")
local utils = require("libraries.utils")
local Viewerutils = require("libraries.viewer_utils")
local WoodBox = require("libraries.WoodBox")

-----------------User Settings------------------
local hasBankPin           = false-- Your Bankpin
-----------------User Settings------------------
...
local aioRune = {
    ...
    { label = "Astral rune", ALTARIDID = 17010, PORTALID = 132613, AREAID = { x = 2158, y = 3864, z = 0 }, RUNEID = 9075 },
...
local abbyRunCounter = 0            -- counts successful abyss runs / trips to altar
local nextDarkMageAt = math.random(0,2) -- visit Dark Mage every 5-7 runs (randomized)
...
local function checkStop()
    if not API.Read_LoopyLoop() then 
        print("Script stop requested, terminating...")
        API.Write_LoopyLoop(false)
        return true
    end
    return false
...
local function waitwalk()
    if checkStop() then return end
    utils.randomSleep(1000)
    repeat
        if checkStop() then return end
        utils.randomSleep(1000)
    until not utils.is_local_player_moving() and not utils.is_local_player_animating() and not API.isProcessing()
end
...
local function RuneToCraft()
    ...
    elseif(aioSelectR.string_value == "Astral rune") then           Rune = "Astral rune"
...
local function SurgeToMage()
    if not (API.ReadPlayerAnim() == ID_Anim.WildyWall) and not API.ReadPlayerMovin2() and (not API.CheckAnim(10)) then
        ...
        if Mage ~= nil and #Mage > 0 then
            ...
            API.logWarn("SurgeToMage: Mage not found nearby")
        end
...
local function startcheck()
     ...
     utils.randomSleep(2500)
        if not API.BankOpen2() then
            ...
            if API.BankOpen2() then
                ...
                Bank:selectEquipmentTab()
                Bank:equip("Grace", true)
                Bank:equip("Wicked hood", true)
    ...
    local function hp()
        local hp = API.GetHPrecent() <  30
         if hp then startcheck() end
    end
   

-- REPLACE existing InnerCircle() with this
local function InnerCircle()
    ...
    print(hp)
    ::retry::
    local mage = (API.PInAreaW((WPOINT.new(3039,4834,0)),21))

    if mage then
        hp()
        -- We're in inner circle vicinity (near mage)
        print("Inner Circle")
        -- print(abbyRunCounter .. "/" .. nextDarkMageAt .. " runs until Dark Mage visit")
        -- If we've hit the threshold, go repair pouches with Dark Mage first
        -- if abbyRunCounter >= nextDarkMageAt then

        local needrepair = (API.Invfreecount_() > 0)
        print("Pouches need repair :" .. tostring(needrepair))
        if needrepair then
            API.logDebug("Info: Dark Mage run triggered. Going to repair pouches before portal.")
            -- Approach / interact with Dark Mage (ID_NPC.MAGE is defined earlier)
            if Interact:NPC("Dark mage", "Repair-pouches") then
                API.RandomSleep2(1000, 500, 1500)
                -- wait until movement/interaction ends
                waitwalk()

                -- Select the repair option(s) in the dialog (user provided sequence)
                API.DoAction_Interface(0xffffffff,0xffffffff,0,1191,15,-1,API.OFF_ACT_GeneralInterface_Choose_option)
                API.RandomSleep2(1000, 1500, 3500)

                API.DoAction_Interface(0xffffffff,0xffffffff,0,1184,15,-1,API.OFF_ACT_GeneralInterface_Choose_option)
                API.RandomSleep2(1000, 1500, 3500)

                API.logInfo("Dark Mage: attempted pouch repair.")
                API.logDebug("Dark Mage: pouch repair sequence executed.")
            else
                API.logWarn("Dark Mage: couldn't interact with NPC; skipping repair this time.")
            end

            -- reset counter and pick next random threshold (5-7 runs)
            abbyRunCounter = 0
            nextDarkMageAt = math.random(5, 7)
            API.logDebug("Next Dark Mage visit in " .. nextDarkMageAt .. " runs.")
        end
        hp()    
        -- Proceed to portal as before
        if API.DoAction_Object1(0x29, API.OFF_ACT_GeneralObject_route0, { selectedPortal }, 50) then
            hp()
            API.RandomSleep2(1500, 3650, 4500)
            waitwalk()
            API.logDebug("Doaction: Shortcut (Inner circle)")

            -- Wait / try to reach altar; loop until we successfully click the altar or script stopped.
            repeat
                API.RandomSleep2(1000, 850, 1500)
                print("waitforaltar")
            until API.DoAction_Object1(0x29, API.OFF_ACT_GeneralObject_route0, { selectedAltar }, 50) or API.Read_LoopyLoop(0)

            -- If we successfully reached the altar (i.e. we clicked it), treat as one completed abyss run
            -- Increment the abbyRunCounter here so we count completed inner-circle -> altar runs
            abbyRunCounter = abbyRunCounter + 1
            API.logDebug("InnerCircle: completed a run. abbyRunCounter = " .. abbyRunCounter) 
        else
            -- If portal click failed, small sleep and fallback handling
            API.RandomSleep2(500, 200, 400)
            waitwalk()
            API.logWarn("InnerCircle: failed to click portal (will retry).")
        end
        ...
        -- Outter Circle — handle obstacles as before
        print("Outter Circle")
        obstacleFound = API.DoAction_Object1(0x29, API.OFF_ACT_GeneralObject_route0, {
            ID_Abby.GAP,
            ID_Abby.TENDRILS,
            ID_Abby.ROCK,
            ID_Abby.EYES,
        ...
        API.RandomSleep2(1500, 1200, 2400)
        until mage or (os.time() - startTime) > 30 or (API.Read_LoopyLoop() == false) or hp
    end
end
            ...
            elseif isAtLocation(AREA.WILDY, 50) then
                ...
                if API.PInArea(3089, 50, 3523, 1) then
                    API.logDebug("Location found: wildy near wall.")
                     DoMage()
                     API.RandomSleep2(2000, 1500, 1650)
                    if SurgeDiveAbillity then
                        SurgeToMage()
                    else
                        WalkToMage()
