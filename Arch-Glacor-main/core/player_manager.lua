...
function PlayerManager:_hasExcalibur()
    ...
    for _, id in ipairs(excalIds) do
        if Inventory:Contains(id) then -- check inventory
            return { location = "inventory", id = id }
        end
        if Equipment:GetOffhand() and Equipment:GetOffhand().id == id then -- check offhand
...
function PlayerManager:_hasElvenShard()
    return Inventory:Contains(43358)
end
...
function PlayerManager:drink(potionId)
    ...
    if not Inventory:InvItemFound(itemId) then return false end
